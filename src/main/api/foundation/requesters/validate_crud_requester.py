...
class ValidateCrudRequester(HttpRequester):
    def __init__(self, request_spec, endpoint, responce_spec):
    ...
    def post(self, model:Optional[BaseModel]=None) -> BaseModel:
        ...
        with allure.step(f"POST {Config.fetch("baclendUrl")}{self.endpoint.value.url} and Validated Model"):
    ...
    def get(self, user_id:Optional[int]=None)->BaseModel:
    ...
    def delete(self, user_id:int):
        ...
        self.responce_spec(response)
        return self.endpoint.value.response_model.model_validate(response)